...
This is done through a factory bean called `PulsarClientFactoryBean`, which takes a client builder object `ClientBuilder`. By default, the application tries to connect to a local Pulsar instance at `pulsar://localhost:6650`. However, there are many application properties available to configure the client.
